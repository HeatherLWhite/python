...
def rSquared(n, sumxy, sumx, sumy, sumxsq, sumysq):
    top = (n * sumxy) - (sumx * sumy)
    bottom = ((n * sumxsq - sumx**2)*(n*sumysq - sumy**2))**0.5
    rsq = (top / bottom)**2
    return rsq
...
for count in range(0, len(yvalues)):
    ...
    params, params_covariance = curve_fit(func, x, yvalues[count], p0=[20000, 9, 0.000001])
    print("Parameters [a b c] for", dataLabels[count], ':', params)
    #print("Covariance for [a b c] for", dataLabels[count], ':', params_covariance)

    xDIVc = []
    for count1 in range(0, len(x)):
        xDIVc.append(x[count1] / params[2])

    xdummyDIVc = []
    for count1 in range(0, len(xdummy)):
        xdummyDIVc.append(xdummy[count1] / params[2])
    ...
    bValue = 4
    cValue = 10e-9
    plt.plot(xdummy, func(xdummy, params[0], params[1], cValue), label=fitLabels[count])
